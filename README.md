...
1. [Обзор реализованных функций «math.h»](#функции-библиотеки)
2. [Спецификация](#спецификация)
3. [Сборка](#сборка)
4. [Юнит тесты](#юнит-тесты)
5. [LCOV report](#lcov-report)
...
- перейдите в `src`
- запустите `make s21_math.a`

команда соберет статическую библиотеку
...
p.s. на MACOS результаты вычислений могут отличаться от Linux, рекомендуется запускать на ОС LINUX

## LCOV report

- перейдите в `src`
- запустите команду `make gcov_report`

![LCOV report](misc/lcov_1.png)
![LCOV report 2](misc/lcov_2.png)
